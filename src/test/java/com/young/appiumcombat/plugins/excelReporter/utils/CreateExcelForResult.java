...
public class CreateExcelForResult{
    	...
    	for (ISuite suite : suites){
    		...
    		for (int i = 0; i < testNum; i++) {
    			...
    			for (int j = 0; j < 6; j++) {
    				...
    				if(j==5){
					//通过率				
 					cell.setCellValue(getPercent(successCases[i], totalCases[i]));
 					cell.setCellStyle(setContentCellBorder(workbook));
    				}		  				
					}		
				}	
	...
	public static XSSFCellStyle setCellStyle4(XSSFWorkbook workbook){
	    ...
	    //居中显示
        xssfCellStyle.setAlignment(CellStyle.VERTICAL_CENTER);
        ...
        xssfFont.setBoldweight(Font.BOLDWEIGHT_BOLD);//加粗      
	...
	public static void setCellBorder(XSSFWorkbook workbook,XSSFCellStyle xssfCellStyle){
		xssfCellStyle.setBorderBottom(CellStyle.BORDER_THIN); //下边框    
		xssfCellStyle.setBorderLeft(CellStyle.BORDER_THIN);//左边框    
		xssfCellStyle.setBorderTop(CellStyle.BORDER_THIN);//上边框    
		xssfCellStyle.setBorderRight(CellStyle.BORDER_THIN);//右边框   
	}
